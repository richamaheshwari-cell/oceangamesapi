...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [items, total] = await Promise.all([
      ...
      prisma.gameArticle.count({ where }),
    ]);
    return ok(res, {
      items,
      page,
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      const article = await prisma.gameArticle.create({
        data: {
...
router.get(
  ...
  requireRole("super_admin", "admin", "editor", "seo_editor"),
  asyncHandler(async (req, res) => {
    const article = await prisma.gameArticle.findUnique({
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    const target = await prisma.gameArticle.findUnique({
      where: { id: req.params.id },
    });
    if (!target) return fail(res, 404, "NOT_FOUND", "Game article not found");
    ...
    try {
      const updated = await prisma.gameArticle.update({
        where: { id: req.params.id },
        data,
        include: includeCreator,
      });
...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    await prisma.gameArticle.delete({ where: { id: req.params.id } });
