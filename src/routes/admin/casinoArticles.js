...
// Optional URL or empty string (coerce "" to null)
const optionalUrl = z
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      const article = await prisma.casinoArticle.create({
        data: {
...
router.get(
  ...
  requireRole("super_admin", "admin", "editor", "seo_editor"),
  asyncHandler(async (req, res) => {
    const article = await prisma.casinoArticle.findUnique({
      where: { id: req.params.id },
      include: includeCreator,
    });
    if (!article)
      return fail(res, 404, "NOT_FOUND", "Casino article not found");
    return ok(res, article);
  }),
);
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    const target = await prisma.casinoArticle.findUnique({
      where: { id: req.params.id },
    });
    if (!target) return fail(res, 404, "NOT_FOUND", "Casino article not found");
    ...
    try {
      const updated = await prisma.casinoArticle.update({
        where: { id: req.params.id },
        data,
        include: includeCreator,
      });
...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    await prisma.casinoArticle.delete({ where: { id: req.params.id } });
